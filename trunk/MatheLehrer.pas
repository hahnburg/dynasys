...
Function TabFkt(x:RR;Var Tab:TList):RR; far;
Var zgr:Integer;
    m:RR;
    p:TPunkt;
Begin
  ...
  Else Begin
    m:=(TPunkt(Tab.items[zgr-1]).y-TPunkt(Tab.items[zgr]).y)/
                       (TPunkt(Tab.items[zgr-1]).x-TPunkt(Tab.items[zgr]).x);
    TabFkt:=TPunkt(Tab.items[zgr-1]).y + m*(x-TPunkt(Tab.items[zgr-1]).x)
End;
End;
